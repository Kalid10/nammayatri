...
putDriverDashboardFleetWmbTripUpdate :: (ShortId DM.Merchant -> City.City -> ApiTokenInfo -> Common.TripTransactionId -> Common.TripPlannerReq -> Flow APISuccess)
putDriverDashboardFleetWmbTripUpdate merchantShortId opCity apiTokenInfo tripTransactionId req = do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  transaction <- buildTransaction apiTokenInfo Nothing (Just req)
  T.withTransactionStoring transaction $ Client.callFleetAPI checkedMerchantId opCity (.driverDSL.putDriverDashboardFleetWmbTripUpdate) tripTransactionId req

deleteDriverDashboardFleetWmbTripDelete :: (ShortId DM.Merchant -> City.City -> ApiTokenInfo -> Common.TripTransactionId -> Flow APISuccess)
deleteDriverDashboardFleetWmbTripDelete merchantShortId opCity apiTokenInfo tripTransactionId = do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  transaction <- buildTransaction apiTokenInfo Nothing (Just tripTransactionId)
  T.withTransactionStoring transaction $ Client.callFleetAPI checkedMerchantId opCity (.driverDSL.deleteDriverDashboardFleetWmbTripDelete) tripTransactionId

--- << AUTOGENERATED Check this code, update export list and remove comment >> ---

putDriverDashboardFleetWmbTripDelete :: (Kernel.Types.Id.ShortId Domain.Types.Merchant.Merchant -> Kernel.Types.Beckn.Context.City -> ApiTokenInfo -> Dashboard.Common.TripTransactionId -> Environment.Flow Kernel.Types.APISuccess.APISuccess)
putDriverDashboardFleetWmbTripDelete merchantShortId opCity apiTokenInfo tripTransactionId = do
  ...
  SharedLogic.Transaction.withTransactionStoring transaction $ (do API.Client.ProviderPlatform.Fleet.callFleetAPI checkedMerchantId opCity (.driverDSL.putDriverDashboardFleetWmbTripDelete) tripTransactionId)
