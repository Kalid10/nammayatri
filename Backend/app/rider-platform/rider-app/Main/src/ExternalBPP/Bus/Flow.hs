...
search merchant merchantOperatingCity bapConfig searchReq = do
  ...
  where
    ...
    mkPass currentTime (eligibilityData, isEligible) =
      let pass = eligibilityData.passData
          passType = eligibilityData.passTypeData
          passCategory = eligibilityData.passCategoryData
          expiryDate = eligibilityData.daysLeft >>= \d -> Just $ addUTCTime (intToNominalDiffTime (d * 86400)) currentTime
       in DPass
            { code = pass.code,
              amount = pass.amount,
              savings = pass.savings,
              benefit = pass.benefit,
              validTripsLeft = eligibilityData.tripsLeft,
