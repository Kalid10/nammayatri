...
validateRideStartedReq RideStartedReq {..} = do
  ...
  unless (booking.status `elem` [DRB.TRIP_ASSIGNED]) $ throwError (BookingInvalidStatus $ show booking.status)
...
validateRideCompletedReq RideCompletedReq {..} = do
  ...
  ride <- QRide.findByBPPRideId bppRideId >>= fromMaybeM (RideDoesNotExist $ "BppRideId" <> bppRideId.getId)
  let bookingCanBeCompleted = booking.status `elem` [DRB.TRIP_ASSIGNED]
