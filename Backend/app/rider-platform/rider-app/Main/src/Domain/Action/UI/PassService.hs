...
postSelect (mbPersonId, merchantId) passId = do
  ...
  case newStatus of
      ...
      pure $
        API.Types.UI.PassService.PassSelectionAPIEntity
          { purchasedPassId = newPersonPass.id,
            paymentOrder = mCreateOrderRes
          }
    Domain.Types.PurchasedPass.Failed -> do
    ...
    _ -> do
      QPurchasedPass.updateStatusByPersonId Domain.Types.PurchasedPass.Failed personId
      throwError $ InternalError "Unexpected response from createOrder"
...
getPurchasedDetail (mbPersonId, merchantId) purchasedPassId = do
  ...
  order <- QOrder.findByShortId paymentOrderShortId >>= fromMaybeM (PaymentOrderNotFound $ getField @"getShortId" purchasedPass.shortId)
